...
const VERY_RAIN_MM_THRESHOLD = 20;
const HOT_TEMP_THRESHOLD_C = Number(process.env.HOT_TEMP_THRESHOLD_C) || 32;
const COLD_TEMP_THRESHOLD_C = Number(process.env.COLD_TEMP_THRESHOLD_C) || 15;
const DEFAULT_GEO = {
  lat: -22.9068,
  lon: -43.1729,
...
function isVeryRainyDay(weather) {
  if (!weather) return false;
  const code = weather.weatherCode;
  if (weather.precipMm >= VERY_RAIN_MM_THRESHOLD) return true;
  if (code != null && code >= 65 && code <= 67) return true;
  if (code != null && code >= 82 && code <= 86) return true;
  return false;
}

function isVeryHotDay(weather) {
  if (!weather || weather.tempMaxC == null) return false;
  return weather.tempMaxC >= HOT_TEMP_THRESHOLD_C;
}

function isVeryColdDay(weather) {
  if (!weather || weather.tempMaxC == null) return false;
  return weather.tempMaxC <= COLD_TEMP_THRESHOLD_C;
}

function enrichDailyWeather(day) {
  return {
    ...day,
    isVeryRainy: isVeryRainyDay(day),
    isVeryHot: isVeryHotDay(day),
    isVeryCold: isVeryColdDay(day),
  };
}
...
async function fetchDailyWeather(lat, lon, startYmd, endYmd) {
  ...
  for (let i = 0; i < days.length; i += 1) {
    ...
    data[days[i]] = enrichDailyWeather({
...
function buildPerVenueGeoDescription({ venuesWithGeo, venueCountTotal }) {
  const n = venuesWithGeo.length;
  if (!n) {
    return `Clima (Open-Meteo) por loja indisponível — nenhuma das ${venueCountTotal} loja(s) selecionada(s) possui coordenada no cadastro.`;
  }
  if (n < venueCountTotal) {
    return `Clima (Open-Meteo) consultado separadamente para ${n} de ${venueCountTotal} lojas com coordenada cadastrada.`;
  }
  return `Clima (Open-Meteo) consultado separadamente para cada uma das ${n} lojas selecionadas.`;
}

async function fetchVenuesDailyWeather(venueGeos, startYmd, endYmd) {
  const withGeo = venueGeos.filter((v) => v.hasGeo);
  const entries = await Promise.all(
    withGeo.map(async (venue) => {
      try {
        const byDay = await fetchDailyWeather(venue.lat, venue.lon, startYmd, endYmd);
        return [venue.id, byDay];
      } catch (err) {
        return [venue.id, { error: err.message }];
      }
    }),
  );
