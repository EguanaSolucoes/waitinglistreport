...
function buildGeoDescription({ hasVenueGeo, venueCountWithGeo, venueCountTotal, region, isFallback }) {
  ...
  if (isFallback) {
    return `Clima (Open-Meteo) para ${place} — referência padrão, pois nenhuma das ${venueCountTotal} venue(s) selecionada(s) possui coordenada no cadastro.`;
  }
  if (venueCountWithGeo < venueCountTotal) {
    return `Clima (Open-Meteo) na região de ${place}, calculado no centróide de ${venueCountWithGeo} de ${venueCountTotal} venues com coordenada cadastrada.`;
  }
  return `Clima (Open-Meteo) na região de ${place}, calculado no centróide das ${venueCountWithGeo} venues selecionadas.`;
}
