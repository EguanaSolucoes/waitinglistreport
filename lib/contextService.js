...
function narrativeForDay(day, baseline) {
  ...
  if (day.hotVenues?.length) {
    parts.push(day.hotVenues.map((v) => `${v.venueName}: calor intenso (${v.weather.tempMaxC}°C)`).join(' · '));
  }
  if (day.coldVenues?.length) {
    parts.push(day.coldVenues.map((v) => `${v.venueName}: frio intenso (${v.weather.tempMaxC}°C máx.)`).join(' · '));
  }
  if (Math.abs(delta) >= 5) {
    parts.push(delta > 0
...
function summarizeDayWeather(venueWeather) {
  ...
  const hotVenues = venueWeather.filter((v) => v.weather?.isVeryHot);
  const coldVenues = venueWeather.filter((v) => v.weather?.isVeryCold);
  ...
  return {
    ...
    isVeryHot: hotVenues.length > 0,
    isVeryCold: coldVenues.length > 0,
    hotVenueCount: hotVenues.length,
    coldVenueCount: coldVenues.length,
    label: maxVenue?.weather?.label || null,
    tempMaxC: maxTemp != null ? round1(maxTemp) : null,
    tempMinC: minTemp != null ? round1(minTemp) : null,
    venueWeather,
    rainyVenues,
    veryRainyVenues,
    hotVenues,
...
async function getContextAnalysis(inicio, fim, venueIds) {
  ...
  const days = ymds.map((ymd, i) => {
    ...
    for (const v of weatherSummary.hotVenues) {
      tags.push({
        type: 'clima',
        name: `${v.venueName} — calor intenso (${v.weather.tempMaxC}°C)`,
        impact: 'negative',
        venueId: v.venueId,
        veryHot: true,
      });
    }
    for (const v of weatherSummary.coldVenues) {
      tags.push({
        type: 'clima',
        name: `${v.venueName} — frio intenso (${v.weather.tempMaxC}°C máx.)`,
        impact: 'negative',
        venueId: v.venueId,
        veryCold: true,
      });
    ...
    return {
      ...
      weather: {
        ...
        isVeryHot: weatherSummary.isVeryHot,
        isVeryCold: weatherSummary.isVeryCold,
        hotVenueCount: weatherSummary.hotVenueCount,
        coldVenueCount: weatherSummary.coldVenueCount,
        label: weatherSummary.label,
        tempMaxC: weatherSummary.tempMaxC,
        tempMinC: weatherSummary.tempMinC,
      },
      ...
      hotVenues: weatherSummary.hotVenues,
      coldVenues: weatherSummary.coldVenues,
      tags,
      isSpecial,
      isWeekend: weekend,
    };
  ...
  const groups = [
    buildGroupStats(days, 'normal', (d) => !d.isSpecial && !d.isWeekend && !d.weather?.isRainy && !d.weather?.isVeryHot && !d.weather?.isVeryCold),
    ...
    buildGroupStats(days, 'calor', (d) => d.weather?.isVeryHot),
    buildGroupStats(days, 'frio', (d) => d.weather?.isVeryCold),
    buildGroupStats(days, 'fim_semana', (d) => d.isWeekend),
  ].filter((g) => g.dayCount > 0);

  const highlights = [...days]
    .filter((d) => d.isSpecial || d.weather?.isRainy || d.weather?.isVeryHot || d.weather?.isVeryCold)
    .sort((a, b) => Math.abs(b.deltaReservasPct) - Math.abs(a.deltaReservasPct))
    .slice(0, 12);

  const weatherAlerts = days.flatMap((day) => (
    (day.venueWeather || [])
      .filter((v) => v.weather?.isRainy || v.weather?.isVeryHot || v.weather?.isVeryCold)
      .map((v) => {
        let message = 'chuva forte';
        let kind = 'chuva';
        if (v.weather.isVeryRainy) {
          message = 'dia muito chuvoso';
          kind = 'chuva';
        } else if (v.weather.isVeryHot) {
          message = 'calor intenso';
          kind = 'calor';
        } else if (v.weather.isVeryCold) {
          message = 'frio intenso';
          kind = 'frio';
        } else if (!v.weather.isRainy) {
          return null;
        }
        return {
          ymd: day.ymd,
          ...
          tempMaxC: v.weather.tempMaxC,
          weatherLabel: v.weather.label,
          isVeryRainy: !!v.weather.isVeryRainy,
          isVeryHot: !!v.weather.isVeryHot,
          isVeryCold: !!v.weather.isVeryCold,
          kind,
          message,
        };
      })
    ...
    const rank = (x) => (x.isVeryRainy ? 0 : x.isVeryHot ? 1 : x.isVeryCold ? 2 : 3);
  ...
  return {
    ...
    sources: {
      ...
      hotTempThresholdC: HOT_TEMP_THRESHOLD_C,
      coldTempThresholdC: COLD_TEMP_THRESHOLD_C,
    },
    meta: {
      venues,
      venueIds: ids,
