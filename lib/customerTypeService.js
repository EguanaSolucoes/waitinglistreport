...
/** Contagem de 1ª visita vs recorrente por loja (para ranking). */
async function aggregateCustomerTypeByVenue(db, venueIds, start, end, collection, dateField, firstMap) {
  const stats = {};
  ...
  for await (const doc of cursor) {
    const vid = doc.venue.toString();
    if (!stats[vid]) stats[vid] = { primeira: 0, recorrente: 0 };
    const firstYmd = firstMap.get(mapKey(doc.customer, doc.venue));
    const isPrimeira = !firstYmd || doc.ymd === firstYmd;
    if (isPrimeira) stats[vid].primeira += 1;
    else stats[vid].recorrente += 1;
  }
  return stats;
}
