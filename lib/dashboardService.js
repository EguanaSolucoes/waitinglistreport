...
async function buildVenueRanking(db, venues, start, end, pStart, pEnd, venueIds) {
  ...
  const [res, resP, resPes, resPesP, wl, wlP, wlPes, wlPesP, tempoCur, tempoPrev] = await Promise.all([
  ...
  return filtered.map((venue) => ({
    ...
    tempo: tempoCur.tempo[venue.id] || 0,
    tempo_p: tempoPrev.tempo[venue.id] || 0,
    tempo_sf: tempoCur.seated[venue.id] || 0,
  }));
}
