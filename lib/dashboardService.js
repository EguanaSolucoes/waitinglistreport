...
const { buildFirstVisitMap, aggregateCustomerType } = require('./customerTypeService');
const {
  aggregateOrigins,
...
async function aggregateWaitlists(db, venueIds, start, end, indexByYmd, indexByYm) {
  ...
  const [byDay, seated, noshow, tempo, hora, horaHeat, pessoas, pessoasSeated, pessoasNoshow, origins, originsDetail, byMonth, seatedMonth, noshowMonth, pessoasMonth, seatedMonthPessoas, noshowMonthPessoas, statusRows] = await Promise.all([
...
async function buildVenueRanking(db, venues, start, end, pStart, pEnd, venueIds) {
  ...
  const [res, resP, resPes, resPesP, wl, wlP, wlPes, wlPesP] = await Promise.all([
  ...
  return filtered.map((venue) => ({
    ...
    res_pes: resPes[venue.id] || 0,
    res_pes_p: resPesP[venue.id] || 0,
    fila: wl[venue.id] || 0,
    fila_p: wlP[venue.id] || 0,
    fila_pes: wlPes[venue.id] || 0,
    fila_pes_p: wlPesP[venue.id] || 0,
  }));
}
...
async function buildPeriodData(db, venueIds, period, prevPeriod) {
  ...
  const [res, resPrev, wl, wlPrev, wi, wiPrev, resFuturo, unidades, canal, resFirstMap, wlFirstMap] = await Promise.all([
  ...
  const [clienteRes, clienteResPrev, clienteFila, clienteFilaPrev] = await Promise.all([
